...
export default function Page() {
  ...
  const shareToKakao = () => {
    if (window.Kakao) {
      ...
      (window.Kakao as any).Share.sendDefault({
        objectType: 'feed',
        content: {
          title: '오현우 & OOO 결혼식에 초대합니다',
          description: '2026년 10월 25일 12시 30분, 해피웨딩홀',
  ...
  const diffDays = Math.ceil((targetMid.getTime() - todayMid.getTime()) / msPerDay)
  ...
  return (
    <>
      ...
      <section className="w-full px-6 pb-8">
        ...
        <div className="mt-4 grid grid-cols-7 gap-2 text-center text-sm font-medium">
          {['일', '월', '화', '수', '목', '금', '토'].map((d, idx) => (
            <div key={d} className={idx === 0 ? 'text-red-500' : 'text-gray-600'}>
              {d}
            </div>
          ))}
        </div>
        ...
        <div className="mt-2 grid grid-cols-7 gap-2 text-center text-sm">
          ...
          {dates.map((d) => {
            ...
            const dayTextClass = weekday === 0 ? 'text-red-500' : 'text-gray-700'
        ...
        <div className="mt-6 text-center">
          ...
          <div className="mt-2 text-xl font-semibold">D-Day {Math.max(diffDays, 0)}</div>
        </div>
      </section>

      <section className="w-full px-6 pb-10">
        <h3 className="text-lg font-medium text-gray-900 text-center">갤러리</h3>
      ...
      <section className="w-full px-6 pb-12">
        <h3 className="text-lg font-medium text-gray-900 text-center">오시는 길</h3>
      ...
      <section className="w-full px-6 pb-14">
        <h3 className="text-lg font-medium text-gray-900 text-center">마음 전하기</h3>
        ...
        <div className="mt-5 space-y-4">
          <div className="rounded-lg bg-gray-50 p-4">
            <div className="text-sm font-semibold text-gray-900">신랑 측 계좌</div>
            <div className="mt-2 text-sm text-gray-700">신한은행</div>

            <div className="mt-2 flex flex-col gap-2">
              <div className="text-sm text-gray-800">110-123-456789</div>
              <button
                type="button"
                onClick={() => handleCopy('110-123-456789')}
            ...
            <div className="mt-2 text-sm text-gray-700">예금주: 김신랑</div>
          </div>

          <div className="rounded-lg bg-gray-50 p-4">
            <div className="text-sm font-semibold text-gray-900">신부 측 계좌</div>
            ...
            <div className="mt-2 text-sm text-gray-700">예금주: 이신부</div>
          </div>
        </div>
      </section>

      <section className="w-full px-6 pb-10">
        <h3 className="text-lg font-medium text-gray-900 text-center">방명록</h3>
              ...
              {guestbookMessages.map((item) => (
                <SwiperSlide key={item.id}>
                  <div className="h-full rounded-xl bg-pink-50 p-4 shadow-sm">
                    <div className="text-sm font-bold text-gray-900">{item.name || '익명'}</div>
      ...
      <section className="w-full px-6 pb-10">
        <h3 className="text-lg font-medium text-gray-900 text-center">청첩장 공유하기</h3>
